/****************************************************************************************************
module명: sCheckAuthCodePerPhone.js
creator: 안상현
date: 2022.9.14
version 1.0
설명: 휴대폰 번호별 인증코드 확인
...
let sCheckAuthCodePerPhone={
    checkAuthCodePerPhone: async function(request, response)
    {
        if(request.body!=undefined&&request.body!=null)
        {
            ...
            //그 다음에 3분 이내에 남은 인증번호를 취득한다.
            tmpStr       ="SELECT a.AUTH_CODE \n";
            tmpStr=tmpStr+"  FROM "+mydb.database+".tb_p00_auth_phone a \n";
            tmpStr=tmpStr+" WHERE a.LOCAL_BIZ_COMP_CODE = '"+myLocalBizCompCode+"' \n";
            tmpStr=tmpStr+"   AND a.AUTH_PHONE = '"+phone+"' \n";
            tmpStr=tmpStr+"   AND a.VALID_YN = 'Y' \n";
            tmpStr=tmpStr+"   AND a.CREATION_DATETIME >= DATE_ADD(SYSDATE(), INTERVAL - 3 MINUTE) \n";
            ...
            try{
                ...
                {
                    //인증 끝났으므로 기존 코드 무효처리
                    await setAuthCodeExpire(phone, inputcode);
                };
        ...
        async function setAuthCodeExpire(phone, inputcode)
        {
            let tmpStr="";
            let mydb = require("./sDbcommonconfigRW");
            const myLocalBizCompCode = mydb.localBizCompCode;
            ...
            tmpStr=tmpStr+"    AND a.AUTH_CODE  ='"+inputcode+"' ";
            let mydao=require("./sDbCommonDaoForCompanyAreaUPD");
            await mydao.sqlHandlerForCompanyUPD(myLocalBizCompCode, "RW", tdevCommon, tmpStr,[]);
        };
    }
...
module.exports=sCheckAuthCodePerPhone;
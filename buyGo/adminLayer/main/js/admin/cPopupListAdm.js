/****************************************************************************************************
module명: cPopupListAdm.js
creator: 정길웅
date: 2022.03.31
version 1.0
설명: 사용자에게 팝업 리스트를 보여주는 화면
...
function reDrawData(pselectedPage){
    eventSearchPopupList(pselectedPage);
};

async function popupListView(pGetSearchCondition){
    //팝업DB 읽어오기
    await $.ajax({
        url: "/uGetPopupListAdm?" + $.param({"authPage" : "cPopupListAdm.js", "searchCondition":pGetSearchCondition}),
        ...
        success: function(response){
            ...
            if(response.errorcode == "PFERR_CONF_0000"){
                ...
                for(var ix=0; ix<returnSetArray.length; ix++){
                    if(returnSetArray[ix].VIEW_YN=="Y"){
                        returnSetArray[ix].VIEW_YN="게시";
                    }
                    else if(returnSetArray[ix].VIEW_YN=="N"){
                        returnSetArray[ix].VIEW_YN="미게시";
                    };

                    if(returnSetArray[ix].VIEW_DIV=="W"){
                        returnSetArray[ix].VIEW_DIV="쇼핑몰";
                    }
                    else if(returnSetArray[ix].VIEW_DIV=="A"){
                        returnSetArray[ix].VIEW_DIV="관리자";
                    };

                    //팝업관리 사이트의 테이블
                    innerText = innerText + `<tr>`;
                    innerText = innerText + `<td>`+start_number+`</td>`;
                    innerText = innerText + `<td><a href="javascript:callLayerPop(${returnSetArray[ix].IDX});">${returnSetArray[ix].TITLE}<a></td>`;
                    innerText = innerText + `<td>${returnSetArray[ix].VIEW_START} ~  ${returnSetArray[ix].VIEW_END}</td>`;
                    innerText = innerText + `<td>${returnSetArray[ix].VIEW_YN}</td>`;
                    innerText = innerText + `<td>${returnSetArray[ix].VIEW_DIV}</td>`;
                    innerText = innerText + `<td>${returnSetArray[ix].REG_DATE}</td>`;
                    innerText = innerText + `<td><button type="button" class="btn_ss01" onClick="previewPop('${returnSetArray[ix].ATTACH_CD}');"><span>미리보기</span></td>`;
                    start_number = start_number-1;
                }
                document.getElementById("adm_popup_list").innerHTML = innerText;
            }
            else{
...
async function textTotalData(pSearchCondition){
    //데이터 총 건수 
    await $.ajax({
        url: "/uGetPopupTotalCntAdm?"  + $.param({"authPage" : "cPopupListAdm.js", "searchCondition":pSearchCondition}),
        type: "POST", async: false,  dataType: "JSON", crossOrigin: true, cache: false,
        contentType: "application/x-www-form-urlencoded; charset=utf-8",
        success: function(response){
...
async function eventSearchPopupList(pGetCurrentPage){

    //search_title : 팝업제목, search_popup_viewYN : 게시상태, search_popup_viewDIV : 게시구분, now_page : 현 보는 페이지, rows_per_page : 한 화면에 보이는 데이터 수
    var getSearchCondition = {
        search_title : $("input[name='search_title']").val(),
        search_popup_viewYN : $("input[name=search_view_yn]:checked").val(), 
        search_popup_viewDIV : $("input[name=search_view_div]:checked").val(),
        now_page : pGetCurrentPage,
        crudGbn : "inquiry_main"
    };
    await textTotalData(getSearchCondition); //검색결과 총 데이터 확인
    await popupListView(getSearchCondition); //검색조건 적용 함수 
};

async function callLayerPop(pIdx){
    await drawPopupListPopUp(pIdx);
};

//팝업창에서 취소 누를 때 layer 사라지게 하는 함수

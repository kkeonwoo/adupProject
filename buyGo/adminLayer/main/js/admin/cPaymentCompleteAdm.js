/****************************************************************************************************
module명: cPaymentCompleteAdm.js
creator: 윤희상
date: 2022.03.25
version 1.0
설명: 관리자 페이지에서 입금완료 화면용 js
...
function drawOrderList(preturnValue, pGetGlobalListCount, pGetCurrentPage){
    ...
    if(preturnValue.errorcode == "PFERR_CONF_0000"){
        ...
        for(var tx=0; tx<returnSetArray.length; tx++){
            ...
            Cell11.innerHTML = (returnSetArray[tx].PAYMENT_POINT).toLocaleString('ko-KR');
            Cell12.innerHTML = `${returnSetArray[tx].BANK_NAME}<br>${returnSetArray[tx].PAYMENT_ACCOUNT}<br>(합계 : <b><span class="comma">${(returnSetArray[tx].PAYMENT_SUM).toLocaleString('ko-KR')}</span></b>)`;
            
            Cell13.innerHTML = returnSetArray[tx].STATUS;
        };
        ...
        $('#orderList tbody').mergeClassRowspan(12, 2);
    }
        ...
        var newRow = tb.insertRow();
        newRow.innerHTML = `<td colspan="14" align="center" style="height:100px;">등록된 데이터가 없습니다</td>`;
    }
    else{

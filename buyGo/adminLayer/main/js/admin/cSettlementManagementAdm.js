/****************************************************************************************************
module명: cSettlementManagementAdm.js
creator: 윤희상
date: 2022.04.12
version 1.0
설명: 관리자 페이지에서 정산관리 화면용 js
...
function drawOrderList(preturnValue, pGetGlobalListCount, pGetCurrentPage){
    ...
    var table = document.getElementsByClassName("ordertable")[0];
    ...
    if(preturnValue.errorcode == "PFERR_CONF_0000"){
        ...
        totInnerHTML+=`<tr> \n`;
        totInnerHTML+=`	<th rowspan="2">주문 건수</th> \n`;
        totInnerHTML+=`	<td rowspan="2"><span class="comma">${(returnTotal[0].ORDER_CNT).toLocaleString('ko-KR')}</span> 건</td> \n`;
        totInnerHTML+=`	<th>주문 금액</th> \n`;
        ...
        totInnerHTML+=`	<th>주문 금액 VAT 미포함</th> \n`;
        ...
        totInnerHTML+=`	<td><span class="comma">${(returnTotal[0].VAT_CANCLE_PRICE_SUM + returnTotal[0].NONVAT_CANCLE_PRICE_SUM).toLocaleString('ko-KR')}</span> 원</td> \n`;
        totInnerHTML+=`	<th>주문취소 VAT 미포함</th> \n`;
        totInnerHTML+=`	<td><span class="comma">${(Math.round(returnTotal[0].VAT_CANCLE_PRICE_SUM/1.1) + returnTotal[0].NONVAT_CANCLE_PRICE_SUM).toLocaleString('ko-KR')}</span> 원</td> \n`;
        totInnerHTML+=`</tr> \n`;
        totInnerHTML+=`<tr> \n`;
        totInnerHTML+=`	<th>포인트 합계</th> \n`;
        totInnerHTML+=`	<td><span class="comma">${(returnTotal[0].POINT_SUM).toLocaleString('ko-KR')}</span> 원</td> \n`;
        totInnerHTML+=`	<th>매출 금액</th> \n`;
        totInnerHTML+=`	<td><span class="comma">${((returnTotal[0].VAT_ORDER_PRICE_SUM + returnTotal[0].NONVAT_ORDER_PRICE_SUM) - (returnTotal[0].VAT_CANCLE_PRICE_SUM + returnTotal[0].NONVAT_CANCLE_PRICE_SUM)).toLocaleString('ko-KR')}</span> 원</td> \n`;
        totInnerHTML+=`	<th>매출 금액 VAT 미포함</th> \n`;
        totInnerHTML+=`	<td><span class="comma">${(Math.round(returnTotal[0].VAT_ORDER_PRICE_SUM/1.1)-Math.round(returnTotal[0].VAT_CANCLE_PRICE_SUM/1.1) + (returnTotal[0].NONVAT_ORDER_PRICE_SUM - returnTotal[0].NONVAT_CANCLE_PRICE_SUM)).toLocaleString('ko-KR')}</span> 원</td> \n`;
        totInnerHTML+=`</tr> \n`;
        ...
        for(var tx=0; tx<returnSetArray.length; tx++){
            ...
            Cell3.innerHTML = returnSetArray[tx].ORDERER_BIZ_NAME;

            Cell4.innerHTML = returnSetArray[tx].RECEIVER_NAME;
            Cell5.innerHTML = returnSetArray[tx].SUPPLIER_NAME;

            if(returnSetArray[tx].GOODS_CHILLED == "0") Cell6.innerHTML = "상온";
            else if(returnSetArray[tx].GOODS_CHILLED == "1") Cell6.innerHTML = "냉장";
            else if(returnSetArray[tx].GOODS_CHILLED == "2") Cell6.innerHTML = "냉동";
            else Cell6.innerHTML = "";

            Cell7.innerHTML = returnSetArray[tx].SALES_ITEM_NAME;
            
            Cell8.innerHTML = returnSetArray[tx].TAX_GB_NM;
        ...
        totInnerHTML+=`<tr> \n`;
        totInnerHTML+=`	<th rowspan="2">주문 건수</th> \n`;
        totInnerHTML+=`	<td rowspan="2"><span class="comma">0</span> 건</td> \n`;
        totInnerHTML+=`	<th>매출 금액</th> \n`;

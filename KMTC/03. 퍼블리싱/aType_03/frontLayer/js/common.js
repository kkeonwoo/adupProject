    ...
    fullpage:{
        ...
        main(){
            ...
            let arrowPath = document.querySelectorAll('.arrowPath'),
                linePath = document.querySelectorAll('.linePath'),
                tlArrow = gsap.timeline({defaults: { duration: 3}}),
                tlLine = gsap.timeline({defaults: { duration: .2}}),
                tlMob = gsap.timeline();

            arrowPath.forEach((el, idx) => {
                let arrowPath = document.getElementById(`mPath0${idx+1}`),
                    arrow = document.getElementById(`arrow0${idx+1}`);
                tlArrow.fromTo(arrowPath, { strokeDashoffset: arrowPath.getTotalLength(), strokeDasharray: arrowPath.getTotalLength() }, { autoAlpha: 1, strokeDashoffset: 0 }, '<')
                .to(arrow, { autoAlpha: 1, motionPath: {path: arrowPath, align: arrowPath, alignOrigin: [.5, .5], autoRotate: true}}, '<')
            })
            linePath.forEach((el, idx) => {
                let path = document.getElementById(`mLinePath${idx+1}`);
                tlLine.fromTo(path, { autoAlpha: 0, strokeDashoffset: path.getTotalLength(), strokeDasharray: path.getTotalLength() }, { autoAlpha: 1, strokeDashoffset: 0 })
            })

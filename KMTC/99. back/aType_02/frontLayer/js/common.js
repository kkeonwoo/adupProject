    ...
    fullpage:{
        ...
        main(){
            let arrowPath = document.querySelectorAll('.arrowPath'),
                tlAsia = gsap.timeline({paused:true, defaults: { autoAlpha: 0, duration: 0.2}}),
                tlMob = gsap.timeline({paused:true, defaults: { duration: 3}})
            ...
            arrowPath.forEach((el, idx) => {
                ...
                tlAsia.fromTo(path, { strokeDashoffset: path.getTotalLength(), strokeDasharray: path.getTotalLength() }, { autoAlpha: 1, strokeDashoffset: 0 })
            })

            tlMob.from('.tooltip_mobile', { y: 100, stagger: {each:0.2}, duration: 1})
        }
    },
    gnb : function(){
